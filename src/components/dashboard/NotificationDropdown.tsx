...
import { Bell, X, Info, CheckCircle2, AlertCircle, Clock, Trash2, Ghost } from 'lucide-react';
    ...
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className={cn(
          ...
          isOpen ? "bg-indigo-600 text-white shadow-lg shadow-indigo-100" : "bg-slate-50 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50"
        )}
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center">
            ...
            <span className="relative inline-flex rounded-full h-4 w-4 bg-red-600 text-[9px] font-black text-white items-center justify-center border-2 border-white">
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className={cn(
                "bg-white z-[100] overflow-hidden flex flex-col shadow-2xl border-l border-slate-100",
            ...
            >
              {/* Header */}
              <div className="p-6 md:p-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
                <div>
                  <h3 className="font-black text-slate-900 text-lg md:text-sm tracking-tight flex items-center gap-2">
                    Notifications
                    {unreadCount > 0 && (
                      <span className="bg-indigo-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full ring-4 ring-indigo-50">
                        {unreadCount}
                      </span>
                    )}
                  </h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Activity & Updates</p>
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => setIsOpen(false)}
                    className="h-10 w-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 hover:text-slate-600 transition-all"
                  >
                    <X size={20} />
                  </button>
                </div>
              ...
              {notifications.length > 0 && (
                <div className="px-6 py-3 md:px-4 md:py-2 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/20">
                {notifications.length > 0 ? (
                    ...
                    {notifications.map((notification) => (
                      <div 
                        ...
                        className={cn(
                          ...
                          !notification.read ? "bg-white shadow-sm ring-1 ring-slate-100 my-1 first:mt-0 last:mb-0" : "opacity-70 grayscale-[0.3] hover:opacity-100 hover:grayscale-0 hover:bg-white"
                        )}
                      >
                        {!notification.read && (
                        ...
                        <div className={cn(
                          ...
                          !notification.read 
                            ? "bg-indigo-50 border-indigo-200 text-indigo-700 font-bold group-hover:scale-105" 
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2 mb-1.5">
                            ...
                            <span className={cn(
                              ...
                              !notification.read ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-400"
                            )}>
                          ...
                          <p className={cn(
                            ...
                            !notification.read ? "text-slate-700 font-bold" : "text-slate-500 font-medium"
                          )}>
                            {notification.message}
                          </p>
                          
                          <div className="flex items-center gap-3 mt-4 md:mt-2">
                            <div className="flex items-center gap-1.5 text-[11px] md:text-[10px] font-bold text-slate-400">
                  ...
                  <div className="h-full flex flex-col items-center justify-center p-12 text-center bg-white">
                    <div className="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center mb-6 text-slate-200 animate-pulse">
                      <Bell size={48} />
                    </div>
                    <h4 className="font-black text-slate-900 text-xl tracking-tight">All Caught Up!</h4>
                    <p className="text-slate-500 mt-2 max-w-[260px] font-medium leading-relaxed">
              ...
              {/* Footer */}
              {notifications.length > 0 && (
                <div className="p-4 md:p-3 border-t border-slate-100 bg-slate-50/30 text-center">
                  <Button variant="ghost" size="sm" className="w-full h-10 md:h-8 text-[11px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-all">
