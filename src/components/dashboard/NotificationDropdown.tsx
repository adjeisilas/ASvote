  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            {/* Backdrop for click away / visual dimming on mobile screens */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setIsOpen(false)}
              className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm z-[998] md:hidden"
            />
            ...
            >
              ...
              {notifications.length > 0 && (
                <div className="px-4 py-2 bg-slate-50 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/20 dark:bg-slate-900/10">
                {notifications.length > 0 ? (
                  <div className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
                    {notifications.map((notification) => (
                      ...
                      >
                        ...
                        <div className={cn(
                          "w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border-2 transition-all shadow-sm",
                          !notification.read 
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2 mb-1.5">
                            <h4 className={cn(
                              "text-xs leading-tight tracking-tight transition-colors break-words font-black",
                              !notification.read ? "text-slate-900 dark:text-slate-100 font-extrabold" : "text-slate-600 dark:text-slate-400"
                            )}>
                              {notification.title}
                            </h4>
                            <span className={cn(
                              "text-[9px] font-black whitespace-nowrap px-1.5 rounded-full py-0.5 shrink-0",
                          ...
                          <p className={cn(
                            "text-[11px] leading-relaxed transition-colors break-words",
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-500">
                              <Clock size={12} className="w-2.5 h-2.5" /> 
                            ...
                            <button 
                              ...
                              className="flex items-center gap-1.5 text-[10px] font-black text-red-500 hover:text-white hover:bg-red-500 px-3 py-1 rounded-lg uppercase tracking-widest ml-auto md:opacity-0 md:group-hover:opacity-100 transition-all border border-red-100 dark:border-red-950 hover:border-red-500 bg-red-50/50 dark:bg-red-950/20"
                            >
                              <Trash2 size={12} className="w-3 h-3" /> <span className="inline">Delete</span>
                            </button>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-slate-900">
                    <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-4 text-slate-200 dark:text-slate-700 animate-pulse">
                      <Bell size={32} />
                    </div>
                    <h4 className="font-black text-slate-900 dark:text-slate-100 text-sm tracking-tight">All Caught Up!</h4>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 max-w-[220px] font-medium leading-relaxed text-xs">
              ...
              {notifications.length > 0 && (
                <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/20 text-center">
                  <Button variant="ghost" size="sm" className="w-full h-8 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all">
