...
import { Vote, LayoutDashboard, LogOut, Menu, X, ChevronRight } from 'lucide-react';
    ...
    <nav className={cn(
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2.5 group">
          <div className="bg-indigo-600 p-2 rounded-xl group-hover:rotate-6 group-hover:scale-105 shadow-md shadow-indigo-600/20 transition-all duration-300">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              <Link to={user?.role === 'admin' ? '/admin' : '/organizer'}>
                <Button variant="ghost" className="hidden sm:flex items-center gap-2 font-black rounded-xl text-xs text-muted-foreground hover:bg-accent/70 h-10 px-4">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <Link to="/login">
                <Button variant="ghost" className="font-black rounded-xl px-5 text-xs text-muted-foreground hover:bg-accent/70 h-10">
                  LOGIN
                </Button>
              </Link>
              <Link to="/register">
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <ChevronRight size={13} className={cn("opacity-40 transition-transform", isActive && "translate-x-0.5 opacity-100 text-indigo-500")} />
                </Link>
              );
            })}
          </div>
          ...
          <div className="pt-6 border-t border-border/60 flex flex-col gap-3">
            ...
            {user && (
              <div className="flex items-center justify-between bg-accent/30 px-4 py-3 rounded-xl">
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider font-extrabold">Notifications</span>
                <NotificationDropdown />
              </div>
            )}
            ...
            {user ? (
              <div className="grid grid-cols-2 gap-2 pt-2">
                ...
                <Button variant="outline" onClick={handleLogout} className="w-full font-extrabold rounded-xl h-11 text-xs text-red-500 hover:bg-red-500/10 border-none uppercase">
                  Logout
                </Button>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-2 pt-2">
                <Link to="/login" onClick={() => setMobileMenuOpen(false)}>
                  <Button variant="outline" className="w-full font-extrabold rounded-xl h-11 text-xs border-border/80 text-foreground uppercase">
                    Login
                  </Button>
                </Link>
